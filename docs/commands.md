...
New in 0.3.1.2, the summary also carries `session_activity[]`: one entry per session in the window, including the sessions that carried no findings, with the tool, file and shell counters the scan already computes and a three-state `retrospective_findings` field (`present`, `absent`, or `unknown` when display omission means absence cannot be established). Every entry carries the full `session_id`, as do `standout[]` and `other_reviewed[]`, so a summary response can be joined to a detail response; session labels are display values and are not unique. Cross-project groups also carry `target_count`, how many distinct files the group's writes touched, which is exact only when both of the scan's omission counters are zero and `null` when either is not. It is absent rather than null on `no_identifiable_project` groups, where the count is not something Reader can know.
